set JOBS;                                            #set of jobs j
        ...
        #ordering
        subject to OrderingConstraint {i in JOBS, j in JOBS: i != j}: StartingTime[j] + processing[j]
<= M*ordering[i, j] + StartingTime[i];
        subject to OrderingConstraint2 {i in JOBS, j in JOBS: i != j}: StartingTime[i] + processing[i]
<= M*(1 - ordering[i, j]) + StartingTime[j];
